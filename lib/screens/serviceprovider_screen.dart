...
import 'package:flutter_health_care_app/controllers/serviceprovider_screen_controller1.dart';
...
class ServiceproviderScreen extends StatefulWidget {
  ...
  ServiceproviderScreen({
    Key key,this.id,this.mainid,this.mainSer,this.subser
  }) : super(key: key);
...
class _ServiceproviderScreen extends State<ServiceproviderScreen> {
  ...
  final regCtrl = Get.put(ServiceProviderController1());
  ...
  void initState() {
    ...
    regCtrl.getData(widget.id,widget.mainid);
  ...
  Widget _category() {
    ...
    child:
        GridView.builder(
            ...
            itemCount: regCtrl.tempservList.length,
            scrollDirection: Axis.vertical,
            itemBuilder: (BuildContext context, int index) {
              return  ServiceCard1(
                  icon: regCtrl.tempservList[index].profileImage, label: regCtrl.tempservList[index].hospitalName);
  ...
  Widget getdoctorWidgetList(Size size) {
    return Column(
        children: regCtrl.tempservList.map((x) {
  ...
  Widget _doctorTile(DocShow model, Size size) {
    return Container(
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          leading: ClipRRect(
            ...
            child: Container(
              ...
              child: Image.network(
                ApiConstants().imagebaseUrl+model.profileImage,
                height: 80,
                width: 50,
                fit: BoxFit.contain,
              ),
          ...
          title: Text(model.hospitalName, style: TextStyles.title.bold),
          subtitle: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
            Text(
        ...
        () {
           Get.to(DetailScreen(model:doctorDataList[0],id:model.hospitalId,fees: model.fees.toString(),mainSer: widget.mainSer,subser: widget.subser,serid: widget.id,subid: widget.mainid));
        },
        borderRadius: BorderRadius.all(Radius.circular(20)),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<ServiceProviderController1>(builder: (_) {
      if (regCtrl.loading) {
        return Scaffold(
      ...
      else if(regCtrl.tempservList.isEmpty){
        return Scaffold(
          ...
          body: Column(
            ...
            children: [
              Center(child:
              Text(
                'Subservice is not aligned to any Hospital',
